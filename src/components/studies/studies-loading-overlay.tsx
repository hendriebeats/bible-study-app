  ...
  return (
    ...
    >
      ...
      {/* Body cover — sits below the header + toolbar row, right of the
          sidebar. `bg-editor-surface` matches the editor's body in the
          loaded state (adapts per theme). `top-21` (84px) = header 48 +
          toolbar 36; keep in sync with the chrome's `min-h-9` toolbar slot. */}
      <div className="absolute top-21 right-0 bottom-0 left-64 bg-editor-surface">
        <BodySkeleton />
      </div>
    </div>
  );
